...
!(function (event) {
  ...
  function processAudio(audioProcessingEvent) {
    songNumber = Module.ccall(
      "mid_song_read_wave",
      "number",
      ["number", "number", "number", "number"],
      [song, generalBuffer1, 2 * audioBufferSize, isPaused]
    );
    if (0 == songNumber) {
      return free();
    }
    ...
    for (let n = Math.pow(2, 15), i = 0; i < audioBufferSize; i++) {
      if (i < songNumber) {
        audioProcessingEvent.outputBuffer.getChannelData(0)[i] = buff[i] =
          Module.getValue(generalBuffer1 + 2 * i, "i16") / n;
      } else {
  ...
  function loadInstruments(midiURL, instrumentURL, missingInstrument) {
    ...
    req.onerror = function () {
      log(
        "Error: Cannot retrieve patch file " + instrumentURL + missingInstrument
      );
    };
    req.onload = function () {
      if (200 != req.status) {
        return log(
          "Error: Cannot retrieve patch file " +
            instrumentURL +
            missingInstrument +
            " : " +
            req.status
      ...
      if (0 == instrumentToBeLoad) {
        let a = Module.ccall(
          ...
          [generalBuffer2, midiAudioData.length, !1]
        );
        let s = 32784;
        let u = Module.ccall(
          "mid_create_options",
          "number",
          ["number", "number", "number", "number"],
          [audioContext.sampleRate, s, 1, 2 * audioBufferSize]
        );
        song = Module.ccall(
          "mid_song_load",
          "number",
  ...
  function loadMidi(midiURL, playingOffset, callback1, callback2) {
    ...
    req.onload = function () {
      if (200 != req.status) {
        return log(
          "Error: Cannot retrieve MIDI file " + midiURL + " : " + req.status
        );
      }
  ...
  function malloc(midiUrl, playingOffset, data, callback1) {
    ...
    instrumentToBeLoad = Module.ccall(
    ...
    if (0 < instrumentToBeLoad) {
      for (let l = 0; l < instrumentToBeLoad; l++) {
        let missingInstrument = Module.ccall(
  ...
  function processMidiData(midiUrl, playingOffset, data, callback1) {
    ...
    let songDuration =
      (Module.ccall("mid_istream_close", "number", ["number"], [o]),
      Module.ccall("mid_song_get_total_time", "number", ["number"], [s]) /
        1000);
  ...
  function silent() {
    if (scriptProcessor) {
      ...
      e.lastChild.setAttribute(
        "src",
        createBaseURL(baseScriptURL) + "silence.mid"
      );
      clearInterval(playingInterval);
      scriptProcessor = 0;
    }
  ...
  function createMidiObject(url) {
    ...
    if (elem) {
      ...
      elem.innerHTML =
        '<object data="' +
        url +
        '" autostart="true" volume="0" type="audio/mid"></object>';
      if (document.body) {
  ...
  function seek(time) {
    if (typeof Module != "undefined") {
      Module.ccall(
        ...
        "void",
        ["number", "number"],
        [song, time * audioContext.sampleRate]
      );
  ...
  try {
    ...
    event.MIDIjs.get_duration = function (url, clbk) {
      if (
        "Microsoft Internet Explorer" == ua.browserName &&
        ua.fullVersion < 10
      ) {
        if (clbk) {
          clbk(-1);
    ...
    if ("WebAudioAPI" == audioMethod) {
      ...
      audioStatus =
        "audioMethod: WebAudioAPI, sampleRate (Hz): " +
        audioContext.sampleRate +
        ", audioBufferSize (Byte): " +
        audioBufferSize;
