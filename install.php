...
if($inputPost->hasValueInstall())
{
    ...
    {
        header("Location: install.php?success=false");
        exit();
    }
}
...
function createUser($input)
{
    ...
    $databaseCredentials->setDriver($input->getDatabaseType());
    $databaseCredentials->setHost($input->getDatabaseHost());
    $databaseCredentials->setPort($input->getDatabasePort());   
    $databaseCredentials->setUsername($input->getDatabaseUser());
    $databaseCredentials->setPassword($input->getDatabasePassword());
    $databaseCredentials->setDatabaseName($input->getDatabaseName());
    $databaseCredentials->setDatabaseSchema($input->getDatabaseSchema());
    $databaseCredentials->setTimeZone();
    ...
    try
    {
        ...
        // use native query because admin_create is noneditable column
        $queryBuilder = new PicoDatabaseQueryBuilder($input->getDatabaseType());
        $sql = $queryBuilder->newQuery()
            ->update("user")
            ->set("admin_create = ? andmin_edit = ? ", $userId, $userId)
...
function installWindows($input)
{
    ...
    if($input->hasValueDatabaseName())
    {
        $dbName = $input->getDatabaseName();
        $commands[] = "SETX APP_DATABASE_NAME \"$dbName\"";
    }
...
function installLinux($input)
{
    ...
    {
        $dbName = $input->getDatabaseName();
        $commands[] = "SETENV APP_DATABASE_NAME \"$dbName\"";
    }
    if($input->hasValueDatabaseUser())
    {
        $dbUser = $input->getDatabaseUser();
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <optgroup label="Indonesia">
                                ...
                                <option value="Asia/Jayapura">Waktu Indonesia Timur</option>
                            </optgroup>
                            ...
                            <optgroup label="America">
                                ...
                                <option value="America/Indiana/Winamec">America/Indiana/Winamec</option>
                                ...
                                <option value="America/Paname">America/Paname</option>
