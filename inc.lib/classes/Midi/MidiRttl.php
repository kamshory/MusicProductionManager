...
class MidiRttl extends Midi
{
	...
	protected $defaultDur;
	protected $defaultScale;
	protected $defaultBpm;
	...
	// CONSTRUCTOR
	//---------------------------------------------------------------
	public function __construct($defaultDur = 4, $defaultScale = 5, $defaultBpm = 63)
	{
	...
	//---------------------------------------------------------------
	protected function getRttl($title = '', $tn = -1) //NOSONAR
	{
		...
		return "$title:d={$this->defaultDur},o={$this->defaultScale},b=" . $this->getBpm() . ":" . implode(',', $commands);
	}

	//---------------------------------------------------------------
	// import RTTL (RTTL2MIDI conversion)
	//---------------------------------------------------------------
	protected function importRttl($rttl, $instrument = 0) //NOSONAR
	{
		...
		foreach ($tones as $tone) {
			preg_match('/^[0-9]*/', $tone, $test); //NOSONAR
			$dur = $test[0];
	...
	//---------------------------------------------------------------
	protected function _findFirstContentTrack()
	{
	...
	//---------------------------------------------------------------
	protected function _checkDotted($quarters)
	{
