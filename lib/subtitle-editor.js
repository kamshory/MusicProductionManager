...
class SrtGenerator {
  constructor(editorCtrl, rawData, path) {
    ...
    this.zoomLevel = [0.125, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2];
    ...
    this.createToolTip = function (tooltipObj) {
      if (tooltipObj) {
        let tooltipId = tooltipObj.getAttribute("aria-describedby");
        if (tooltipId) {
          document.querySelector(
            "#" + tooltipId + " .tooltip-inner"
          ).innerText = dis.ddToDmsSrtFormat(seconds / 3600);
        }
      }
    };

    this.hilightText = function (index) {
      if (
        this.diplayedText != "" &&
    ...
    this.createEditorContainer = function () {
      ...
      mapController.style.width =
        this.msToUnit(this.durationMinimum) + this.unit;
    ...
    this.appendController = function (srtData, index, afterElement) {
      ...
      if (afterElement) {
        ...
        this.editorMapElement
          .querySelector(".srt-container")
    ...
    this.movableElement = function (
      ...
      mover.addEventListener("dblclick", function (e) {
        let index = parseInt(
          dis.closest(e.target, ".srt-controller").getAttribute("data-index")
        );
      ...
      dis.getEditor().addEventListener(
        "mousemove",
        function (e) {
          if (dis.isMouseDown) {
            if (dis.editMode == "move") {
              dis.elementToEdit.style.left =
                e.clientX + dis.currentDataOffsetLeft + "px";
            }
      ...
      if (index >= 0 && this.data.length > 0) {
        if (index == 0) {
          ...
          if (this.data[index].start < this.data[prevIndex].start) {
            ...
            this.data[prevIndex].end =
              this.data[prevIndex].start + this.data[prevIndex].duration;
            this.data[index].start = this.data[prevIndex].end;
            this.data[index].end =
              this.data[index].start + this.data[index].duration;
          } else if (this.data[index].start < this.data[prevIndex].end) {
            this.data[prevIndex].end = this.data[index].start;
            this.data[prevIndex].duration =
              this.data[prevIndex].end - this.data[prevIndex].start;
            this.data[index].end =
              this.data[index].start + this.data[index].duration;
          }
    ...
    this.sundulKananMove = function (index) {
      if (this.data.length > 1 && index < this.data.length - 1) {
        ...
        if (
          this.data[index].start + this.data[index].duration >
          this.data[nextIndex].start + this.data[nextIndex].duration
        ) {
          ...
          this.data[index].start + this.data[index].duration >
          this.data[nextIndex].start
        ) {
          // kena sundul
          if (this.data[index].start < this.data[nextIndex].start) {
            // masih bisa resize current data
            this.data[index].duration =
              this.data[nextIndex].start - this.data[index].start;
            this.updateController(index);
          } else if (this.data[index].start < this.data[nextIndex].end) {
            // resize next data
            this.data[nextIndex].start =
              this.data[index].start + this.data[index].duration;
            this.data[nextIndex].duration =
              this.data[nextIndex].end - this.data[nextIndex].start;
    ...
    this.changeOffset = function(offset)
    {
        for(let i in this.data)
        {
            this.data[i].start = this.data[i].start + offset;
            this.data[i].end = this.data[i].end + offset;
            this.updateController(parseInt(i));
        }
        this.updateRawData();
    ...
    this.updateData = function () {
      ...
      for (let i in elems) {
        ...
        if (
          ...
          elem.classList.contains("srt-controller")
        ) {
          let text = elem
            .querySelector(".srt-object-inner .srt-text-container")
            .innerText.trim();
          let timestamp = this.getTimestampFromController(
            elem.offsetLeft,
            elem.style.width
          );
          let start = timestamp.start;
    ...
    this.createControllerContainer = function (srtData, index) {
      ...
      controller
        .querySelector(".srt-text-edit")
        .addEventListener("blur", function (e) {
          dis.updateData();
          dis
            .closest(e.target, ".srt-list-item")
      ...
      controller
        ...
        .addEventListener("keydown", function (e) {
          if (
            ...
            let start1 = thisController.querySelector(
              ".srt-time-edit-start"
            ).value;
            ...
            if (
              index2 == lastIndex &&
              endMs1 + durationMs1 < dis.duration * 1000
            ) {
    ...
    this.updateTextController = function (index, text) {
      // find controller
      dis.querySelector(
        '.srt-controller[data-index="' + index + '"] .srt-text-container'
      ).innerText = text;
    };
    ...
    this.getChunkSize = function () {
      return Math.round(
        this.sampleRate / (1000 * this.resolution * this.scale)
      );
    };
    ...
    this.updatePointerPosition = function (left) {
      // trim container
      left = left - 3;
      this.querySelector(".srt-time-position-pointer").style.left =
        left + this.unit;
    };
    ...
    this.initEditor = function () {
      ...
      this.querySelector(".srt-zoom-control").value =
        this.zoomLevelIndexOriginal;
      this.querySelector(".srt-zoom-control").addEventListener(
        "change",
        function (e) {
      ...
      this.querySelector(".srt-timeline-canvas").addEventListener(
        "contextmenu",
        function (e) {
          e.preventDefault();
          e.stopPropagation();
          ...
          let pos =
            e.clientX + dis.querySelector(".srt-map").scrollLeft - offset;
      ...
      this.querySelector(".srt-time-position-pointer").addEventListener(
        "contextmenu",
        function (e) {
          e.preventDefault();
          e.stopPropagation();
          let offset = parseFloat(
            dis.closest(e.target, ".srt-time-position-pointer").style.left
          );
          let pos = offset;
          let seconds = dis.unitToMs(pos) / 1000;
          let x = pos + 24;
      ...
      this.querySelector(".srt-text-raw").addEventListener(
        "paste",
        function (e) {
          e.preventDefault();
          e.stopPropagation();
