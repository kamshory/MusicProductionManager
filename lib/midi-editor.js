...
class MidiEditor {
    constructor(editorCtrl, rawData, path) {
        ...
        this.createMidiObject = function(code, note, velocity, start, end, duration, text)
        {
            return {
                code:code,
                note:note,
        ...
        this.createData = function (tsData, text) {
            if (tsData) {
                ...
                */
                let code = 0, note = 'A0', velocity = 0;
                return this.createMidiObject(code, note, velocity, start, end, duration, text);
            }
            /*
            return {
                start: 0,
            ...
            */
            let code = 0, note = 'A0', velocity = 0;
            return this.createMidiObject(code, note, velocity, 0, 0, 0, text);
        };
        ...
        this.sundulKiriMove = function (index) {
            if (index >= 0 && this.data.length > 0) {
                if (index == 0) {
                    ...
                    if(this.data[index].code != this.data[prevIndex].code)
                    {
                        return;
                    }
        ...
        this.sundulKananMove = function (index) {
            if (this.data.length > 1 && index < this.data.length - 1) {
                ...
                if(this.data[index].code != this.data[nextIndex].code)
                {
                    return;
                }              
        ...
        this.updateData = function () {
            ...
            for (let i in elems) {
                ...
                if (typeof elem.className != 'undefined' && elem.classList.contains('srt-controller')) {
                    ...
                    */
                    let code = 0, note = 'A0', velocity = 0;
                    newSrt.push(this.createMidiObject(code, note, velocity, start, end, duration, text));
                }
            }
            this.data = newSrt;
            this.updateRawData();
        ...
        this.createControllerContainer = function (srtData, index) {
            ...
            controller.querySelector('.srt-text-edit').addEventListener('keydown', function (e) {
                if ((e.key === 'Enter' || e.keyCode === 13) && (!e.shiftKey && !e.altKey && !e.ctrlKey)) {
                    ...
                    let code = 0, note = 'A0', velocity = 0;
                    let srtData1 = this.createMidiObject(code, note, velocity, startMs1, endMs1, durationMs1, text1);

                  
                    let srtData2 = this.createMidiObject(code, note, velocity, startMs2, endMs2, durationMs2, text2);
